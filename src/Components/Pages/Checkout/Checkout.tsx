...
const dispatch=useAppDispatch()
  ...
  useEffect(() => {
    if (bookingData?.statusCode === 200) {
      ...
      swal("Success", bookingData.message, "success");
      dispatch(clearCart())
      setFormData(inetialValue)
        ...
        <form onSubmit={bookingHandle} className="flex items-start gap-5 justify-center">
          ...
          <div className="w-1/2 p-11 pt-0 text-center">
            ...
            <div className="flex gap-3 mt-4">
              <input
                defaultChecked
                className="bg-black scale-150"
                id="cod"
                type="radio"
              />
              <label htmlFor="cod">
                {" "}
                <img className="w-[50px]" src={cod} alt="cashOnDeliver" />
              </label>
            </div>

            <Button
              text="Place Order"
              className="text-xl font-normal lg:px-24 mt-5 text-center "
            />
          </div>
        </form>
      </div>
